...
import { trackAddToCart, trackInitiateCheckout, trackPurchase } from './metaEvents';
import { shippingFor } from './shipping';

const CartContext = createContext(null);

export function CartProvider({ children, brandId, deliveryRule = null }) {
  ...
  const addToCart = (product, size, color = null) => {
    trackAddToCart({
      id: product.id ?? productIdFromSlug(product.slug) ?? product.slug,
      title: product.title,
  ...
  const placeOrder = async (brandId, details) => {
    ...
    const cartValue = cartItems.reduce((total, item) => total + item.price * item.qty, 0);
    trackInitiateCheckout({ items: cartItems, value: cartValue });
    ...
    try {
      ...
      // Reported before the cart is cleared, because the sale is described by
      // what was in it.
      trackPurchase({
        orderId: body.data.orderId,
        items: cartItems,
        value: body.data.totalAmount,
      });
  ...
  return (
    <CartContext.Provider value={{
      ...
      favorites,
      orders,
      // The platform's rule, so every place that quotes delivery quotes the
      // same number the order is built from.
      deliveryRule,
      openCart,
      closeCart,
      addToCart,
